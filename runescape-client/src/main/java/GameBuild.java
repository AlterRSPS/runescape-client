		...
		for (var4 = 0; var4 < Players.Players_count; ++var4) { // L: 76
			...
			if ((Players.field1244[var19] & 1) == 0) { // L: 78
				if (var3 > 0) { // L: 79
					...
					updateBlock = var0.readBits(1); // L: 84
					if (updateBlock == 0) { // L: 85
			...
			for (var4 = 0; var4 < Players.Players_count; ++var4) { // L: 98
				...
				if ((Players.field1244[var19] & 1) != 0) { // L: 100
					if (var3 > 0) { // L: 101
						...
						updateBlock = var0.readBits(1); // L: 106
						if (updateBlock == 0) { // L: 107
				...
				for (var4 = 0; var4 < Players.Players_emptyIdxCount; ++var4) { // L: 120
					...
					if ((Players.field1244[var19] & 1) != 0) { // L: 122
						if (var3 > 0) { // L: 123
							...
							updateBlock = var0.readBits(1); // L: 128
							if (updateBlock == 0) { // L: 129
				...
				if (var3 != 0) { // L: 138
					...
					for (var4 = 0; var4 < Players.Players_emptyIdxCount; ++var4) { // L: 142
						...
						if ((Players.field1244[var19] & 1) == 0) { // L: 144
							if (var3 > 0) { // L: 145
								...
								updateBlock = var0.readBits(1); // L: 150
								if (updateBlock == 0) { // L: 151
					...
					if (var3 != 0) { // L: 160
						...
						for (var3 = 0; var3 < Players.Players_pendingUpdateCount; ++var3) { // L: 173
							...
							updateBlock = var0.readUnsignedByte(); // L: 176

							if ((updateBlock & 128) != 0) { // Excess Flag
								updateBlock += var0.readUnsignedByte() << 8;
							}

							byte var7 = -1; // L: 179
							// face pawn
							if ((updateBlock & 4) != 0) { // L: 180
								var5.targetIndex = var0.readUShortAdd(); // L: 181
								if (var5.targetIndex == 65535) { // L: 182
									var5.targetIndex = -1;
								}
							}
							// face tile
							if ((updateBlock & 32) != 0) { // L: 184
								var5.field1112 = var0.readUShortAdd(); // L: 185
							...
							// public chat
							if ((updateBlock & 16) != 0) {
								System.out.println("______::::PUBLIC CHAT MASK:::____");
								var8 = var0.readUShortAdd();
								System.out.println("Var8: " + var8);

								PlayerType var9 = (PlayerType)UserComparator4.findEnumerated(VarbitComposition.PlayerType_values(), var0.readUByteNeg());
								boolean var10 = var0.readUnsignedByte() == 1;
								System.out.println("Autochat? Var10: " + var10);
								...
								if (var5.username != null && var5.appearance != null) {
									...
									if (!var13 && Client.field539 == 0 && !var5.isHidden) { // L: 202
										...
										var0.N_readBytesAdd(Players.field1240.array, 0, var11); // L: 204
							...
							// Force Chat
							if ((updateBlock & 64) != 0) { // L: 222
								var5.overheadText = var0.readString(); // L: 223
							...
							int var17;
							// ANIMATION
							if ((updateBlock & 8) != 0) { // L: 247
								var8 = var0.readUShortAdd(); // L: 248
								if (var8 == 65535) { // L: 249
									var8 = -1;
								}

								var17 = var0.readUByteNeg(); // L: 250
								FaceNormal.performPlayerAnimation(var5, var8, var17); // L: 251
							}
							//  force move
							if ((updateBlock & 512) != 0) { // L: 253
								var5.field1083 = var0.readByteAdd(); // L: 254
								var5.field1100 = var0.readByte(); // L: 255
								var5.field1128 = var0.readByte(); // L: 256
								var5.field1130 = var0.readByte(); // L: 257
								var5.field1131 = var0.readUShortLE() + Client.cycle; // L: 258
								var5.field1132 = var0.readUShortAdd() + Client.cycle; // L: 259
							...
							if ((updateBlock & 4096) != 0) { // L: 277
								var7 = var0.readByteNeg(); // L: 278
								System.out.println("MovementBlock:var7: " + var7);
							}
							// gfx
							if ((updateBlock & 256) != 0) { // L: 280
								var5.spotAnimation = var0.readUShortLE(); // L: 281
							...
							// Full movement
							if ((updateBlock & 2048) != 0) { // L: 290
								Players.field1241[var4] = var0.readByteNeg(); // L: 291
							}
							// Hitmark
							if ((updateBlock & 2) != 0) { // L: 293
								var8 = var0.readUByteSub(); // L: 294
								int var18;
								int var21;
								int var23;
								if (var8 > 0) { // L: 295
								...
								if (var17 > 0) { // L: 317
									for (var21 = 0; var21 < var17; ++var21) { // L: 318
										...
										if (var12 != 32767) { // L: 321
											...
											var18 = var0.readUByteSub(); // L: 323
											var15 = var12 > 0 ? var0.readUByteSub() : var18; // L: 324
