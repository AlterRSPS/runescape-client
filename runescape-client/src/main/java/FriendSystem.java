			...
			if (var2.hasCleanName()) { // L: 77
				if (this.friendsListIsFull()) { // L: 78
					...
					if (HealthBarDefinition.localPlayer.username.equals(var2)) { // L: 82
						...
						PacketBufferNode var3 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.FRIENDLIST_ADD, Client.packetWriter.isaacCipher); // L: 106
						var3.packetBuffer.writeByte(Client.stringCp1252NullTerminatedByteSize(var1)); // L: 107
						var3.packetBuffer.sendStringByBytes(var1); // L: 108
		...
		if (var1 != null) { // L: 195
			...
			if (var2.hasCleanName()) { // L: 197
				if (this.friendsList.removeByUsername(var2)) {
					VarcInt.method3033();
					PacketBufferNode var3 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.FRIENDLIST_DEL, Client.packetWriter.isaacCipher); // L: 201
					var3.packetBuffer.writeByte(Client.stringCp1252NullTerminatedByteSize(var1));
					var3.packetBuffer.sendStringByBytes(var1);
		...
		if (var1 != null) { // L: 226
			...
			if (var2.hasCleanName()) { // L: 228
				if (this.ignoreList.removeByUsername(var2)) { // L: 229
					...
					PacketBufferNode var3 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IGNORELIST_DEL, Client.packetWriter.isaacCipher); // L: 232
					var3.packetBuffer.writeByte(Client.stringCp1252NullTerminatedByteSize(var1)); // L: 233
					var3.packetBuffer.sendStringByBytes(var1); // L: 234
