	...
	static void method5169(int var0, String var1) {
		...
		for (int var6 = 0; var6 < var2; ++var6) { // L: 9079
			...
			if (var7 != null && var7 != HealthBarDefinition.localPlayer && var7.username != null && var7.username.equals(var5)) { // L: 9081
				...
				if (var0 == 1) { // L: 9082
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER1, Client.packetWriter.isaacCipher); // L: 9084
					...
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER4, Client.packetWriter.isaacCipher); // L: 9091
					var8.packetBuffer.writeShortAdd(var3[var6]); // L: 9092
					var8.packetBuffer.writeByteAdd(0); // L: 9093
					Client.packetWriter.addNode(var8); // L: 9094
				} else if (var0 == 6) { // L: 9096
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER6, Client.packetWriter.isaacCipher); // L: 9098
					var8.packetBuffer.writeShortLE(var3[var6]); // L: 9099
					var8.packetBuffer.wirteByteNeg(0); // L: 9100
					Client.packetWriter.addNode(var8); // L: 9101
				} else if (var0 == 7) { // L: 9103
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER7, Client.packetWriter.isaacCipher); // L: 9105
					var8.packetBuffer.writeShort(var3[var6]); // L: 9106
					var8.packetBuffer.writeByteAdd(0); // L: 9107
