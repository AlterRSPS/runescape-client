...
public class class14 {
	...
	@Export("menuAction")
	static final void menuAction(int var0_slot, int var1_hash, int var2, int var3_item, String var4, String var5, int var6, int var7) {
		if (var2 >= 2000) { // L: 8273
			var2 -= 2000;
		}
		...
		if (var2 == 1) { // L: 8274
			...
			Client.destinationX = var0_slot; // L: 8721
			Client.destinationY = var1_hash; // L: 8722
			var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOCU, Client.packetWriter.isaacCipher); // L: 8724
			var8.packetBuffer.writeShortLE(ReflectionCheck.clickedSlot); // L: 8725
			var8.packetBuffer.writeShortLE(DefaultsGroup.baseY + var1_hash); // L: 8726
			var8.packetBuffer.N_writeIntLE(MouseHandler.component); // L: 8727
			var8.packetBuffer.writeShortAdd(var0_slot + class19.baseX); // L: 8728
			var8.packetBuffer.writeShortLE(var3_item); // L: 8729
			var8.packetBuffer.writeShortAddLE(SoundSystem.item); // L: 8730
			var8.packetBuffer.wirteByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8731
			...
			Client.destinationX = var0_slot; // L: 8461
			Client.destinationY = var1_hash; // L: 8462
			var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOCT, Client.packetWriter.isaacCipher); // L: 8463
			var8.packetBuffer.N_writeIntME(FontName.hash); // L: 8464
			var8.packetBuffer.writeShortAdd(Client.spellSlot); // L: 8465
			var8.packetBuffer.writeShortAdd(var0_slot + class19.baseX); // L: 8466
			var8.packetBuffer.writeShortLE(Client.verify); // L: 8467
			var8.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8468
			var8.packetBuffer.writeShortAdd(var3_item); // L: 8469
			var8.packetBuffer.writeShortAddLE(DefaultsGroup.baseY + var1_hash); // L: 8470
			...
			Client.destinationX = var0_slot; // L: 8898
			Client.destinationY = var1_hash; // L: 8899
			var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOC1, Client.packetWriter.isaacCipher); // L: 8901
			var8.packetBuffer.writeShortLE(var0_slot + class19.baseX); // L: 8902
			var8.packetBuffer.writeShort(DefaultsGroup.baseY + var1_hash); // L: 8903
			var8.packetBuffer.writeShortAdd(var3_item); // L: 8904
			...
			var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOC2, Client.packetWriter.isaacCipher); // L: 8962
			var8.packetBuffer.writeShortLE(var0_slot + class19.baseX); // L: 8963
			var8.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8964
			var8.packetBuffer.writeShort(var3_item); // L: 8965
			var8.packetBuffer.writeShortAddLE(DefaultsGroup.baseY + var1_hash); // L: 8966
			...
			Client.destinationX = var0_slot; // L: 8623
			Client.destinationY = var1_hash; // L: 8624
			var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOC3, Client.packetWriter.isaacCipher); // L: 8626
			var8.packetBuffer.writeShort(var0_slot + class19.baseX); // L: 8627
			var8.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8628
			var8.packetBuffer.writeShort(DefaultsGroup.baseY + var1_hash); // L: 8629
			var8.packetBuffer.writeShortAdd(var3_item); // L: 8630
			...
			Client.destinationX = var0_slot; // L: 8321
			Client.destinationY = var1_hash; // L: 8322
			var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOC4, Client.packetWriter.isaacCipher); // L: 8324
			var8.packetBuffer.writeShortAddLE(var3_item); // L: 8325
			var8.packetBuffer.writeShort(DefaultsGroup.baseY + var1_hash); // L: 8326
			var8.packetBuffer.writeShortAddLE(var0_slot + class19.baseX); // L: 8327
			var8.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8328
			Client.packetWriter.addNode(var8); // L: 8329
		} else {
			PacketBufferNode var9;
			NPC var13;
			if (var2 == 7) {
				var13 = Client.npcs[var3_item]; // L: 8383
				if (var13 != null) { // L: 8384
					...
					Client.destinationX = var0_slot; // L: 8389
					Client.destinationY = var1_hash; // L: 8390
					var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPCU, Client.packetWriter.isaacCipher); // L: 8392
					var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8393
					var9.packetBuffer.writeShortAdd(SoundSystem.item); // L: 8394
					var9.packetBuffer.writeShortLE(ReflectionCheck.clickedSlot); // L: 8395
					var9.packetBuffer.N_writeIntME(MouseHandler.component); // L: 8396
					var9.packetBuffer.writeShortAddLE(var3_item); // L: 8397
					Client.packetWriter.addNode(var9); // L: 8398
				}
			} else if (var2 == 8) {
				var13 = Client.npcs[var3_item]; // L: 8876
				if (var13 != null) { // L: 8877
					...
					Client.destinationX = var0_slot; // L: 8882
					Client.destinationY = var1_hash; // L: 8883
					var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPCT, Client.packetWriter.isaacCipher); // L: 8884
					var9.packetBuffer.N_writeIntIME(FontName.hash); // component_hash
					var9.packetBuffer.writeShortAdd(Client.spellSlot); // component_slot
					var9.packetBuffer.wirteByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // MOVEMENT TYPE
					var9.packetBuffer.writeShortAdd(var3_item); // npc_index
					var9.packetBuffer.writeShort(Client.verify);
					Client.packetWriter.addNode(var9); // L: 8890
				}
			} else if (var2 == 9) {
				var13 = Client.npcs[var3_item]; // L: 8603
				if (var13 != null) { // L: 8604
					...
					Client.destinationX = var0_slot; // L: 8609
					Client.destinationY = var1_hash; // L: 8610
					var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPC1, Client.packetWriter.isaacCipher); // L: 8612
					var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8613
					var9.packetBuffer.writeShortLE(var3_item); // L: 8614
					Client.packetWriter.addNode(var9); // L: 8615
				}
			} else if (var2 == 10) {
				var13 = Client.npcs[var3_item]; // L: 9022
				if (var13 != null) { // L: 9023
					...
					Client.destinationX = var0_slot; // L: 9028
					Client.destinationY = var1_hash; // L: 9029
					var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPC2, Client.packetWriter.isaacCipher); // L: 9031
					var9.packetBuffer.writeShortLE(var3_item); // L: 9032
					var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9033
					Client.packetWriter.addNode(var9); // L: 9034
				}
			} else if (var2 == 11) {
				var13 = Client.npcs[var3_item]; // L: 8833
				if (var13 != null) { // L: 8834
					...
					Client.destinationX = var0_slot; // L: 8839
					Client.destinationY = var1_hash; // L: 8840
					var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPC3, Client.packetWriter.isaacCipher); // L: 8842
					var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8843
					var9.packetBuffer.writeShortAdd(var3_item); // L: 8844
					Client.packetWriter.addNode(var9); // L: 8845
				}
			} else if (var2 == 12) {
				var13 = Client.npcs[var3_item]; // L: 8518
				if (var13 != null) { // L: 8519
					...
					Client.destinationX = var0_slot; // L: 8524
					Client.destinationY = var1_hash; // L: 8525
					var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPC4, Client.packetWriter.isaacCipher); // L: 8527
					var9.packetBuffer.writeShortLE(var3_item); // L: 8528
					var9.packetBuffer.wirteByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8529
					Client.packetWriter.addNode(var9); // L: 8530
				}
			} else if (var2 == 13) {
				var13 = Client.npcs[var3_item]; // L: 8634
				if (var13 != null) { // L: 8635
					...
					Client.destinationX = var0_slot; // L: 8640
					Client.destinationY = var1_hash; // L: 8641
					var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPC5, Client.packetWriter.isaacCipher); // L: 8643
					var9.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8644
					var9.packetBuffer.writeShort(var3_item); // L: 8645
					Client.packetWriter.addNode(var9); // L: 8646
				}
			} else {
				Player var15;
				if (var2 == 14) {
					var15 = Client.players[var3_item]; // L: 8792
					if (var15 != null) { // L: 8793
						...
						Client.destinationX = var0_slot; // L: 8798
						Client.destinationY = var1_hash; // L: 8799
						var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYERU, Client.packetWriter.isaacCipher); // L: 8801
						var9.packetBuffer.writeShortAdd(var3_item); // L: 8802
						var9.packetBuffer.writeShortAdd(SoundSystem.item); // L: 8803
						var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8804
						var9.packetBuffer.writeShortAdd(ReflectionCheck.clickedSlot); // L: 8805
						var9.packetBuffer.N_writeIntIME(MouseHandler.component); // L: 8806
						Client.packetWriter.addNode(var9); // L: 8807
					}
				} else if (var2 == 15) {
					var15 = Client.players[var3_item]; // L: 8751
					if (var15 != null) { // L: 8752
						...
						Client.destinationX = var0_slot; // L: 8757
						Client.destinationY = var1_hash; // L: 8758
						var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYERT, Client.packetWriter.isaacCipher); // L: 8759
						var9.packetBuffer.writeShortAdd(Client.verify); // L: 8760
						var9.packetBuffer.writeShortAddLE(var3_item); // player index
						var9.packetBuffer.writeShortAddLE(Client.spellSlot); // L: 8762
						var9.packetBuffer.writeInt(FontName.hash); // L: 8763
						var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // keydown
					...
					Client.destinationX = var0_slot; // L: 8703
					Client.destinationY = var1_hash; // L: 8704
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJU, Client.packetWriter.isaacCipher); // L: 8706
					var8.packetBuffer.writeShortAddLE(var3_item); // ground item
					var8.packetBuffer.writeShort(DefaultsGroup.baseY + var1_hash); // Z
					var8.packetBuffer.writeShort(SoundSystem.item); // L: 8709
					var8.packetBuffer.writeShort(var0_slot + class19.baseX); // L: 8710
					var8.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8711
					var8.packetBuffer.writeShortAddLE(ReflectionCheck.clickedSlot); // L: 8712
					var8.packetBuffer.N_writeIntLE(MouseHandler.component); // L: 8713
					...
					Client.destinationX = var0_slot; // L: 8654
					Client.destinationY = var1_hash; // L: 8655
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJT, Client.packetWriter.isaacCipher); // L: 8656
					var8.packetBuffer.wirteByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8657
					var8.packetBuffer.writeShortAddLE(Client.verify); // L: 8658
					var8.packetBuffer.writeShortAdd(var3_item); // L: 8659
					var8.packetBuffer.writeShortLE(DefaultsGroup.baseY + var1_hash); // L: 8660
					var8.packetBuffer.writeInt(FontName.hash); // L: 8661
					var8.packetBuffer.writeShortLE(Client.spellSlot); // L: 8662
					var8.packetBuffer.writeShortAddLE(var0_slot + class19.baseX); // L: 8663
					...
					Client.destinationX = var0_slot; // L: 8491
					Client.destinationY = var1_hash; // L: 8492
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJ1, Client.packetWriter.isaacCipher); // L: 8494
					var8.packetBuffer.writeShortAdd(var0_slot + class19.baseX); // L: 8495
					var8.packetBuffer.writeShort(var3_item); // L: 8496
					var8.packetBuffer.writeShort(DefaultsGroup.baseY + var1_hash); // L: 8497
					var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8498
					...
					Client.destinationX = var0_slot; // L: 8913
					Client.destinationY = var1_hash; // L: 8914
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJ2, Client.packetWriter.isaacCipher); // L: 8916
					var8.packetBuffer.writeShortAddLE(DefaultsGroup.baseY + var1_hash); // L: 8917
					var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8918
					var8.packetBuffer.writeShortLE(var3_item); // L: 8919
					var8.packetBuffer.writeShort(var0_slot + class19.baseX); // L: 8920
					...
					Client.destinationX = var0_slot; // L: 8435
					Client.destinationY = var1_hash; // L: 8436
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJ3, Client.packetWriter.isaacCipher); // L: 8438
					var8.packetBuffer.writeShortAdd(DefaultsGroup.baseY + var1_hash); // L: 8439
					var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8440
					var8.packetBuffer.writeShortAdd(var0_slot + class19.baseX); // L: 8441
					var8.packetBuffer.writeShortAdd(var3_item); // L: 8442
					...
					Client.destinationX = var0_slot; // L: 8406
					Client.destinationY = var1_hash; // L: 8407
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJ4, Client.packetWriter.isaacCipher); // L: 8409
					var8.packetBuffer.writeShortAdd(DefaultsGroup.baseY + var1_hash); // L: 8410
					var8.packetBuffer.writeShort(var3_item); // L: 8411
					var8.packetBuffer.writeShortAddLE(var0_slot + class19.baseX); // L: 8412
					var8.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8413
					...
					Client.destinationX = var0_slot; // L: 8306
					Client.destinationY = var1_hash; // L: 8307
					var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJ5, Client.packetWriter.isaacCipher); // L: 8309
					var8.packetBuffer.writeByteSub(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8310
					var8.packetBuffer.writeShortAdd(var0_slot + class19.baseX); // L: 8311
					var8.packetBuffer.writeShortAddLE(var3_item); // L: 8312
					var8.packetBuffer.writeShort(DefaultsGroup.baseY + var1_hash); // L: 8313
					Client.packetWriter.addNode(var8); // L: 8314
				} else if (var2 == 23) {
					if (Client.isMenuOpen) { // L: 8362
						GameBuild.scene.setViewportWalking(); // L: 8363
					} else {
						GameBuild.scene.menuOpen(GrandExchangeOfferTotalQuantityComparator.Client_plane, var0_slot, var1_hash, true); // L: 8366
					}
				} else {
					PacketBufferNode var11;
					Widget var16;
					if (var2 == 24) {
						var16 = UserComparator9.getWidget(var1_hash); // L: 8937
						...
						if (var12) { // L: 8940
							var11 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.BUTTON_CLICK, Client.packetWriter.isaacCipher); // L: 8942
							var11.packetBuffer.writeInt(var1_hash); // L: 8943
							Client.packetWriter.addNode(var11); // L: 8944
						}
					} else {
						if (var2 == 25) {
							var16 = ItemContainer.getWidgetChild(var1_hash, var0_slot); // L: 8924
							if (var16 != null) { // L: 8925
								class262.Widget_runOnTargetLeave(); // L: 8926
								Decimator.selectSpell(var1_hash, var0_slot, SecureRandomCallable.Widget_unpackTargetMask(PendingSpawn.getWidgetFlags(var16)), var16.item); // L: 8927
						...
						if (var2 == 26) {
							...
							Widget var14;
							if (var2 == 28) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.BUTTON_CLICK, Client.packetWriter.isaacCipher); // L: 8781
								var8.packetBuffer.writeInt(var1_hash); // L: 8782
								Client.packetWriter.addNode(var8); // L: 8783
								var14 = UserComparator9.getWidget(var1_hash); // L: 8784
								...
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.BUTTON_CLICK, Client.packetWriter.isaacCipher); // L: 8418
								var8.packetBuffer.writeInt(var1_hash); // L: 8419
								Client.packetWriter.addNode(var8); // L: 8420
								var14 = UserComparator9.getWidget(var1_hash); // L: 8421
								...
								if (Client.meslayerContinueWidget == null) { // L: 8948
									class92.resumePauseWidget(var1_hash, var0_slot); // L: 8949
									Client.meslayerContinueWidget = ItemContainer.getWidgetChild(var1_hash, var0_slot); // L: 8950
									class184.invalidateWidget(Client.meslayerContinueWidget); // L: 8951
								}
							} else if (var2 == 31) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELDU, Client.packetWriter.isaacCipher); // L: 8971
								var8.packetBuffer.writeShortAdd(var3_item); // to item
								var8.packetBuffer.N_writeIntLE(var1_hash); // from component
								var8.packetBuffer.writeShort(var0_slot); // to slot
								var8.packetBuffer.N_writeIntLE(MouseHandler.component); // to component
								var8.packetBuffer.writeShortAddLE(SoundSystem.item); // from item
								var8.packetBuffer.writeShortAdd(ReflectionCheck.clickedSlot); // from slot

								Client.packetWriter.addNode(var8); // L: 8978
								Client.field576 = 0; // L: 8979
								WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8980
								Client.field577 = var0_slot; // L: 8981
							} else if (var2 == 32) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELDT, Client.packetWriter.isaacCipher); // L: 9039
								var8.packetBuffer.writeShortAddLE(var0_slot); // slot
								var8.packetBuffer.writeInt(FontName.hash);	// to hash
								var8.packetBuffer.writeShort(Client.spellSlot);	// spell_slot
								var8.packetBuffer.N_writeIntLE(var1_hash);	// hash
								var8.packetBuffer.writeShortAdd(var3_item); // item

								Client.packetWriter.addNode(var8);
								Client.field576 = 0;
								WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 9047
								Client.field577 = var0_slot; // L: 9048
							} else if (var2 == 33) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELD1, Client.packetWriter.isaacCipher); // L: 8546
								var8.packetBuffer.writeShortAddLE(var3_item); // L: 8547
								var8.packetBuffer.writeShortLE(var0_slot); // L: 8548
								var8.packetBuffer.N_writeIntIME(var1_hash); // L: 8549
								Client.packetWriter.addNode(var8); // L: 8550
								Client.field576 = 0; // L: 8551
								WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8552
								Client.field577 = var0_slot; // L: 8553
							} else if (var2 == 34) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELD2, Client.packetWriter.isaacCipher); // L: 8276
								var8.packetBuffer.writeShortAdd(var3_item); // L: 8277
								var8.packetBuffer.writeShortAdd(var0_slot); // L: 8278
								var8.packetBuffer.N_writeIntIME(var1_hash); // L: 8279
								Client.packetWriter.addNode(var8); // L: 8280
								Client.field576 = 0; // L: 8281
								WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8282
								Client.field577 = var0_slot; // L: 8283
							} else if (var2 == 35) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELD3, Client.packetWriter.isaacCipher); // L: 9001
								var8.packetBuffer.writeShortAddLE(var3_item); // L: 9002
								var8.packetBuffer.writeShortAdd(var0_slot); // L: 9003
								var8.packetBuffer.N_writeIntIME(var1_hash); // L: 9004
								Client.packetWriter.addNode(var8); // L: 9005
								Client.field576 = 0; // L: 9006
								WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 9007
								Client.field577 = var0_slot; // L: 9008
							} else if (var2 == 36) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELD4, Client.packetWriter.isaacCipher); // L: 8689
								var8.packetBuffer.writeShortAddLE(var3_item); // L: 8690
								var8.packetBuffer.writeShortLE(var0_slot); // L: 8691
								var8.packetBuffer.N_writeIntME(var1_hash); // L: 8692
								Client.packetWriter.addNode(var8); // L: 8693
								Client.field576 = 0; // L: 8694
								WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8695
								Client.field577 = var0_slot; // L: 8696
							} else if (var2 == 37) {
								var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELD5, Client.packetWriter.isaacCipher); // L: 8535
								var8.packetBuffer.writeShortAdd(var0_slot); // L: 8536
								var8.packetBuffer.writeShort(var3_item); // L: 8537
								var8.packetBuffer.N_writeIntLE(var1_hash); // L: 8538
								Client.packetWriter.addNode(var8); // L: 8539
								Client.field576 = 0; // L: 8540
								WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8541
								Client.field577 = var0_slot; // L: 8542
							} else {
								if (var2 == 38) {
									class262.Widget_runOnTargetLeave(); // L: 8849
									var16 = UserComparator9.getWidget(var1_hash); // L: 8850
									Client.isItemSelected = 1; // L: 8851
									ReflectionCheck.clickedSlot = var0_slot; // L: 8852
									MouseHandler.component = var1_hash; // L: 8853
									SoundSystem.item = var3_item; // L: 8854
									class184.invalidateWidget(var16); // L: 8855
									Client.selectedItemName = MouseHandler.colorStartTag(16748608) + TaskHandler.ItemDefinition_get(var3_item).name + MouseHandler.colorStartTag(16777215); // L: 8856
								...
								if (var2 == 39) {
									var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IF1_BUTTON1, Client.packetWriter.isaacCipher); // L: 8770
									var8.packetBuffer.writeShortLE(var3_item); // L: 8771
									var8.packetBuffer.writeShort(var0_slot); // L: 8772
									var8.packetBuffer.writeInt(var1_hash); // L: 8773
									Client.packetWriter.addNode(var8); // L: 8774
									Client.field576 = 0; // L: 8775
									WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8776
									Client.field577 = var0_slot; // L: 8777
								} else if (var2 == 40) {
									var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IF1_BUTTON2, Client.packetWriter.isaacCipher); // L: 9012
									var8.packetBuffer.writeInt(var1_hash); // L: 9013
									var8.packetBuffer.writeShort(var3_item); // L: 9014
									var8.packetBuffer.writeShortLE(var0_slot); // L: 9015
									Client.packetWriter.addNode(var8); // L: 9016
									Client.field576 = 0; // L: 9017
									WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 9018
									Client.field577 = var0_slot; // L: 9019
								} else if (var2 == 41) {
									var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IF1_BUTTON3, Client.packetWriter.isaacCipher); // L: 8447
									var8.packetBuffer.N_writeIntIME(var1_hash); // L: 8448
									var8.packetBuffer.writeShortAddLE(var0_slot); // L: 8449
									var8.packetBuffer.writeShort(var3_item); // L: 8450
									Client.packetWriter.addNode(var8); // L: 8451
									Client.field576 = 0; // L: 8452
									WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8453
									Client.field577 = var0_slot; // L: 8454
								} else if (var2 == 42) {
									var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IF1_BUTTON4, Client.packetWriter.isaacCipher); // L: 8668
									var8.packetBuffer.writeShort(var3_item); // L: 8669
									var8.packetBuffer.N_writeIntIME(var1_hash); // L: 8670
									var8.packetBuffer.writeShort(var0_slot); // L: 8671
									Client.packetWriter.addNode(var8); // L: 8672
									Client.field576 = 0; // L: 8673
									WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8674
									Client.field577 = var0_slot; // L: 8675
								} else if (var2 == 43) {
									var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IF1_BUTTON5, Client.packetWriter.isaacCipher); // L: 8349
									var8.packetBuffer.writeInt(var1_hash); // L: 8350
									var8.packetBuffer.writeShort(var0_slot); // L: 8351
									var8.packetBuffer.writeShortAddLE(var3_item); // L: 8352
									Client.packetWriter.addNode(var8); // L: 8353
									Client.field576 = 0; // L: 8354
									WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8355
									Client.field577 = var0_slot; // L: 8356
								} else if (var2 == 44) {
									var15 = Client.players[var3_item]; // L: 8502
									if (var15 != null) { // L: 8503
										...
										Client.destinationX = var0_slot; // L: 8508
										Client.destinationY = var1_hash; // L: 8509
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER1, Client.packetWriter.isaacCipher); // L: 8511
										var9.packetBuffer.writeShort(var3_item); // L: 8512
										var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8513
										Client.packetWriter.addNode(var9); // L: 8514
									}
								} else if (var2 == 45) {
									var15 = Client.players[var3_item]; // L: 8332
									if (var15 != null) { // L: 8333
										...
										Client.destinationX = var0_slot; // L: 8338
										Client.destinationY = var1_hash; // L: 8339
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER2, Client.packetWriter.isaacCipher); // L: 8341
										var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8342
										var9.packetBuffer.writeShortAdd(var3_item); // L: 8343
										Client.packetWriter.addNode(var9); // L: 8344
									}
								} else if (var2 == 46) {
									var15 = Client.players[var3_item]; // L: 9051
									if (var15 != null) { // L: 9052
										...
										Client.destinationX = var0_slot; // L: 9057
										Client.destinationY = var1_hash; // L: 9058
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER3, Client.packetWriter.isaacCipher); // L: 9060
										var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 9061
										var9.packetBuffer.writeShort(var3_item); // L: 9062
										Client.packetWriter.addNode(var9); // L: 9063
									}
								} else if (var2 == 47) {
									var15 = Client.players[var3_item]; // L: 8984
									if (var15 != null) { // L: 8985
										...
										Client.destinationX = var0_slot; // L: 8990
										Client.destinationY = var1_hash; // L: 8991
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER4, Client.packetWriter.isaacCipher); // L: 8993
										var9.packetBuffer.writeShortAdd(var3_item); // L: 8994
										var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8995
										Client.packetWriter.addNode(var9); // L: 8996
									}
								} else if (var2 == 48) {
									var15 = Client.players[var3_item]; // L: 8556
									if (var15 != null) { // L: 8557
										...
										Client.destinationX = var0_slot; // L: 8562
										Client.destinationY = var1_hash; // L: 8563
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER5, Client.packetWriter.isaacCipher); // L: 8565
										var9.packetBuffer.writeShortAddLE(var3_item); // L: 8566
										var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8567
										Client.packetWriter.addNode(var9); // L: 8568
									}
								} else if (var2 == 49) {
									var15 = Client.players[var3_item]; // L: 8286
									if (var15 != null) { // L: 8287
										...
										Client.destinationX = var0_slot; // L: 8292
										Client.destinationY = var1_hash; // L: 8293
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER6, Client.packetWriter.isaacCipher); // L: 8295
										var9.packetBuffer.writeShortLE(var3_item); // L: 8296
										var9.packetBuffer.wirteByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8297
										Client.packetWriter.addNode(var9); // L: 8298
									}
								} else if (var2 == 50) {
									var15 = Client.players[var3_item]; // L: 8817
									if (var15 != null) { // L: 8818
										...
										Client.destinationX = var0_slot; // L: 8823
										Client.destinationY = var1_hash; // L: 8824
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER7, Client.packetWriter.isaacCipher); // L: 8826
										var9.packetBuffer.writeShort(var3_item); // L: 8827
										var9.packetBuffer.writeByteAdd(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8828
										Client.packetWriter.addNode(var9); // L: 8829
									}
								} else if (var2 == 51) {
									var15 = Client.players[var3_item]; // L: 8735
									if (var15 != null) { // L: 8736
										...
										Client.destinationX = var0_slot; // L: 8741
										Client.destinationY = var1_hash; // L: 8742
										var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPPLAYER8, Client.packetWriter.isaacCipher); // L: 8744
										var9.packetBuffer.writeByte(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8745
										var9.packetBuffer.writeShortAddLE(var3_item); // L: 8746
									...
									label646: {
										if (var2 != 57) {
											if (var2 == 58) {
												var16 = ItemContainer.getWidgetChild(var1_hash, var0_slot); // L: 8572
												if (var16 != null) { // L: 8573
													var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IF_BUTTONT, Client.packetWriter.isaacCipher); // L: 8575
													var9.packetBuffer.writeShortAdd(var16.item); // L: 8576
													var9.packetBuffer.writeInt(FontName.hash); // L: 8577
													var9.packetBuffer.writeShortAdd(var0_slot); // L: 8578
													var9.packetBuffer.writeInt(var1_hash); // L: 8579
													var9.packetBuffer.writeShortAdd(Client.verify); // L: 8580
													var9.packetBuffer.writeShortAdd(Client.spellSlot); // L: 8581
											...
											if (var2 == 1001) {
												...
												Client.destinationX = var0_slot; // L: 8865
												Client.destinationY = var1_hash; // L: 8866
												var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOC5, Client.packetWriter.isaacCipher); // L: 8868
												var8.packetBuffer.writeShortAdd(var0_slot + class19.baseX); // L: 8869
												var8.packetBuffer.writeShortAddLE(var3_item); // L: 8870
												var8.packetBuffer.writeShortAddLE(DefaultsGroup.baseY + var1_hash); // L: 8871
												var8.packetBuffer.wirteByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? 1 : 0); // L: 8872
											...
											if (var2 == 1002) {
												...
												var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPLOC6, Client.packetWriter.isaacCipher); // L: 8683
												var8.packetBuffer.writeShortLE(var3_item); // L: 8684
											...
											if (var2 == 1003) {
												...
												var13 = Client.npcs[var3_item]; // L: 8590
												if (var13 != null) { // L: 8591
													...
													if (var17 != null) { // L: 8594
														var11 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPNPC6, Client.packetWriter.isaacCipher); // L: 8596
											...
											if (var2 == 1004) {
												...
												var8 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJ6, Client.packetWriter.isaacCipher); // L: 8378
												var8.packetBuffer.writeShortAddLE(var3_item); // L: 8379
												Client.packetWriter.addNode(var8); // L: 8380
												break label646;
											}

											if (var2 == 1005) {
												var16 = UserComparator9.getWidget(var1_hash); // L: 8474
												if (var16 != null && var16.itemQuantities[var0_slot] >= 100000) { // L: 8475
													class397.addGameMessage(27, "", var16.itemQuantities[var0_slot] + " x " + TaskHandler.ItemDefinition_get(var3_item).name);
												} else {
													var9 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPOBJ6, Client.packetWriter.isaacCipher); // L: 8478
													var9.packetBuffer.writeShortAddLE(var3_item); // L: 8479
													Client.packetWriter.addNode(var9); // L: 8480
												}

												Client.field576 = 0; // L: 8482
												WorldMapSection2.field2037 = UserComparator9.getWidget(var1_hash); // L: 8483
												Client.field577 = var0_slot; // L: 8484
												break label646;
											}

											if (var2 != 1007) {
												if (var2 == 1011 || var2 == 1010 || var2 == 1009 || var2 == 1008 || var2 == 1012) {
													class133.worldMap.worldMapMenuAction(var2, var3_item, new Coord(var0_slot), new Coord(var1_hash)); // L: 8370
												}
												break label646;
											}
										}

										var16 = ItemContainer.getWidgetChild(var1_hash, var0_slot); // L: 8811
										if (var16 != null) { // L: 8812
											GrandExchangeEvents.widgetDefaultMenuAction(var3_item, var1_hash, var0_slot, var16.item, var5); // L: 8813
										}
									}
								}
							}
		...
		if (Client.isItemSelected != 0) { // L: 9066
			...
			class184.invalidateWidget(UserComparator9.getWidget(MouseHandler.component)); // L: 9068
		}
	...
	static final void method191() {
		PacketBufferNode var0 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.CLOSE_MODAL, Client.packetWriter.isaacCipher); // L: 11505
