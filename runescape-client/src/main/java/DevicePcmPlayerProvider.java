					...
					if (var16 == 3) { // L: 9422
						...
						if (var34 != null) { // L: 9424
							for (TileItem var33 = (TileItem)var34.first(); var33 != null; var33 = (TileItem)var34.next()) { // L: 9425 9426 9459
								ItemComposition var37 = TaskHandler.ItemDefinition_get(var33.itemID); // L: 9427
								if (Client.isItemSelected == 1) { // L: 9428
									EnumComposition.insertMenuItemNoShift("Use", Client.selectedItemName + " " + "->" + " " + MouseHandler.colorStartTag(16748608) + var37.name, 16, var33.itemID, var13, var14); // L: 9429
								} else if (Client.isSpellSelected) { // L: 9432
									if ((class116.selectedSpellFlags & 1) == 1) { // L: 9433
										EnumComposition.insertMenuItemNoShift(Client.selectedSpellActionName, Client.selectedSpellName + " " + "->" + " " + MouseHandler.colorStartTag(16748608) + var37.name, 17, var33.itemID, var13, var14); // L: 9434
									}
								} else {
									...
									for (int var30 = 4; var30 >= 0; --var30) { // L: 9440
										if (var29 != null && var29[var30] != null) { // L: 9441
											...
											EnumComposition.insertMenuItemNoShift(var29[var30], MouseHandler.colorStartTag(16748608) + var37.name, var23, var33.itemID, var13, var14); // L: 9448
										} else if (var30 == 2) { // L: 9451
											EnumComposition.insertMenuItemNoShift("Take", MouseHandler.colorStartTag(16748608) + var37.name, 20, var33.itemID, var13, var14); // L: 9452
										}
									}

									EnumComposition.insertMenuItemNoShift("Examine", MouseHandler.colorStartTag(16748608) + var37.name, 1004, var33.itemID, var13, var14); // L: 9456
								}
							}
						}
					}
