...
public final class Client extends GameEngine implements Usernamed, OAuthTokens {
				...
				if (!var70) { // L: 935
					...
					if (!var70) { // L: 953
						...
						if (ArchiveLoader.client.hasFrame()) { // L: 1509
							var63 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.DETECT_MODIFIED_CLIENT, packetWriter.isaacCipher); // L: 1511
						...
						var63 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.MAP_BUILD_COMPLETE, packetWriter.isaacCipher); // L: 1531
			...
			if (loginState == 5) { // L: 2544
				...
				if (gameState == 40) { // L: 2560
					...
					if (field500.method7220()) { // L: 2587
						...
						var22.sendStringByBytes(this.field501); // L: 2589
					} else {
						var22.writeByte(class404.field4351.rsOrdinal()); // L: 2592
						var22.sendStringByBytes(Login.Login_password); // L: 2593
					}
				}
				...
				var6.packetBuffer.sendStringByBytes(Login.Login_username); // L: 2622
				...
				var6.packetBuffer.sendStringByBytes(UserComparator3.field1316); // L: 2635
				...
				var6.packetBuffer.N_writeIntLE(Messages.archive6.hash); // L: 2642
				var6.packetBuffer.writeInt(Archive.archive15.hash); // L: 2643
				var6.packetBuffer.N_writeIntIME(SecureRandomCallable.archive14.hash); // L: 2644
				var6.packetBuffer.N_writeIntME(HorizontalAlignment.archive7.hash); // L: 2645
				var6.packetBuffer.N_writeIntLE(GrandExchangeOfferOwnWorldComparator.archive20.hash); // L: 2646
				var6.packetBuffer.N_writeIntIME(Message.archive17.hash); // L: 2647
				var6.packetBuffer.N_writeIntIME(GrandExchangeOfferOwnWorldComparator.archive8.hash); // L: 2648
				var6.packetBuffer.N_writeIntLE(Messages.archive18.hash); // L: 2649
				var6.packetBuffer.N_writeIntME(class1.archive2.hash); // L: 2650
				var6.packetBuffer.N_writeIntLE(0); // L: 2651
				var6.packetBuffer.N_writeIntLE(class14.archive12.hash); // L: 2652
				var6.packetBuffer.N_writeIntLE(SecureRandomFuture.archive9.hash); // L: 2653
				...
				var6.packetBuffer.N_writeIntLE(ChatChannel.archive10.hash); // L: 2656
				var6.packetBuffer.writeInt(class11.archive5.hash); // L: 2657
				var6.packetBuffer.N_writeIntIME(class111.archive19.hash); // L: 2658
				var6.packetBuffer.N_writeIntIME(class12.archive4.hash); // L: 2659
				var6.packetBuffer.N_writeIntIME(ParamComposition.archive3.hash); // L: 2660
			...
			if (loginState == 13) { // L: 2790
				...
				if (loginState != 16) { // L: 2857
					...
					if (loginState == 18 && ((AbstractSocket)var1).available() >= class122.field1420) { // L: 2985 2986
						...
						String var27 = var2.readString(); // L: 2990
						String var30 = var2.readString(); // L: 2991
						String var28 = var2.readString(); // L: 2992
				...
				while (true) {
					...
					if (!var32) { // L: 3317
						...
						if (timer.field3900) { // L: 3325
							var14 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.LOGIN_STATISTICS, packetWriter.isaacCipher); // L: 3327
								...
								for (var7 = 0; var7 < Interpreter.mouseRecorder.index && (var33 == null || var33.packetBuffer.offset - var3 < 246); ++var7) { // L: 3345 3346
									...
									if (var9 != field475 || var8 != field476) { // L: 3354
										if (var33 == null) { // L: 3357
											var33 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.EVENT_MOUSE_MOVE, packetWriter.isaacCipher); // L: 3358
						...
						if (MouseHandler.MouseHandler_lastButton == 1 || !ApproximateRouteStrategy.mouseCam && MouseHandler.MouseHandler_lastButton == 4 || MouseHandler.MouseHandler_lastButton == 2) { // L: 3425
							...
							var18 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.EVENT_MOUSE_CLICK, packetWriter.isaacCipher); // L: 3436
						...
						if (KeyHandler.field110 > 0) { // L: 3442
							var14 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.EVENT_KEYBOARD, packetWriter.isaacCipher); // L: 3443
							...
							for (var5 = 0; var5 < KeyHandler.field110; ++var5) { // L: 3447
								...
								var14.packetBuffer.N_writeMediumLME((int)var21); // last
								var14.packetBuffer.writeByteSub(KeyHandler.field125[var5]); // KEY
							}
						...
						if (field555 && field620 <= 0) { // L: 3459
							...
							var14 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.EVENT_CAMERA_POSITION, packetWriter.isaacCipher); // L: 3463
							var14.packetBuffer.writeShortLE(camAngleX); // pitch (10 uses)
							var14.packetBuffer.writeShortAddLE(camAngleY); // yaw (14 uses)
							packetWriter.addNode(var14); // L: 3466
						}

						if (WorldMapSectionType.hasFocus && !hadFocus) { // L: 3468
							hadFocus = true; // L: 3469
							var14 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.EVENT_APPLET_FOCUS, packetWriter.isaacCipher); // L: 3471
						...
						if (!WorldMapSectionType.hasFocus && hadFocus) { // L: 3475
							...
							var14 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.EVENT_APPLET_FOCUS, packetWriter.isaacCipher); // L: 3478
						...
						while (true) {
							...
							do {
								...
								if (var51 == null) { // L: 3748
									while (true) {
										do {
													...
													do {
														...
														if (var51 == null) { // L: 3772
																...
																if (MouseHandler.MouseHandler_currentButton == 0) { // L: 3793
																	if (field667) { // L: 3794
																		if (class274.hoveredItemContainer == class247.dragInventoryWidget && dragItemSlotDestination != dragItemSlotSource) { // L: 3795
																			...
																			var18 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.OPHELD_D, packetWriter.isaacCipher); // L: 3826
																			var18.packetBuffer.writeShortLE(dragItemSlotDestination); // L: 3827
																			var18.packetBuffer.writeShortAddLE(dragItemSlotSource); // L: 3828
																			var18.packetBuffer.wirteByteNeg(var39); // INSERT MODE
																			var18.packetBuffer.N_writeIntLE(class247.dragInventoryWidget.hash); // L: 3830
															...
															if (Scene.shouldSendWalk()) { // L: 3862
																...
																var18 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.MOVE_GAMECLICK, packetWriter.isaacCipher); // L: 3866
																var18.packetBuffer.writeByte(5); // L: 3867
																var18.packetBuffer.writeShortAdd(var4 + class19.baseX); // X
																var18.packetBuffer.wirteByteNeg(KeyHandler.KeyHandler_pressedKeys[82] ? (KeyHandler.KeyHandler_pressedKeys[81] ? 2 : 1) : 0); // movement_type
																var18.packetBuffer.writeShortAddLE(var5 + DefaultsGroup.baseY); // Z
															...
															if (var4 > 15000 && var6 > 15000) { // L: 4021
																...
																var29 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IDLE_LOGOUT, packetWriter.isaacCipher); // L: 4025
															...
															if (packetWriter.pendingWrites > 50) { // L: 4030
																var29 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.NO_TIMEOUT, packetWriter.isaacCipher); // L: 4032
														...
														if (var52.slot < 0) { // L: 3774
															break;
														}

														var46 = UserComparator9.getWidget(var52.parentId); // L: 3775
													} while(var46 == null || var46.children == null || var52.slot >= var46.children.length || var52 != var46.children[var52.slot]); // L: 3776
											...
											if (var52.slot < 0) { // L: 3762
												break;
											}

											var46 = UserComparator9.getWidget(var52.parentId); // L: 3763
										} while(var46 == null || var46.children == null || var52.slot >= var46.children.length || var52 != var46.children[var52.slot]); // L: 3764
								...
								if (var52.slot < 0) { // L: 3750
									break;
								}

								var46 = UserComparator9.getWidget(var52.parentId); // L: 3751
							} while(var46 == null || var46.children == null || var52.slot >= var46.children.length || var52 != var46.children[var52.slot]); // L: 3752

							Tile.runScriptEvent(var51); // L: 3756
						}
					}

					var33 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.REFLECTION_CHECK_REPLY, packetWriter.isaacCipher); // L: 3318
	...
	final boolean method1180(PacketWriter var1) {
		...
		if (var2 == null) { // L: 6029
			...
			try {
				...
				int var17;
				String var53;
				if (ServerPacket.SET_PLAYER_OP == var1.serverPacket) { // L: 6070
					var17 = var3.readUByteSub(); // INDEX
					var5 = var3.readUByteNeg(); // LEFT CLICK
					var53 = var3.readString(); // OPTION
					System.out.println("SET_PLAYER_OP");
				...
				int var9;
				int var10;
				if (ServerPacket.CAM_LOOKAT == var1.serverPacket) { // L: 6082
				...
				if (ServerPacket.UPDATE_RUNWEIGHT == var1.serverPacket) { // L: 6105
				...
				if (ServerPacket.TRADING_POST_UPDATE == var1.serverPacket) { // L: 6112
				...
				if (ServerPacket.AREA_SOUND == var1.serverPacket) { // L: 6126
					class21.processZoneUpdate(ZoneUpdate.AREA_SOUND); // L: 6127
					var1.serverPacket = null; // L: 6128
					return true; // L: 6129
				}

				byte var61;
				if (ServerPacket.VARP_SMALL == var1.serverPacket) { // L: 6131
					var61 = var3.readByte(); // L: 6132
					var5 = var3.readUShortAdd(); // L: 6133
				...
				if (ServerPacket.OBJ_DEL == var1.serverPacket) { // L: 6143
					class21.processZoneUpdate(ZoneUpdate.OBJ_DEL); // L: 6144
					var1.serverPacket = null; // L: 6145
					return true; // L: 6146
				}

				if (ServerPacket.CAM_SHAKE == var1.serverPacket) { // L: 6148
				...
				int var15;
				String var25;
				if (ServerPacket.MESSAGE_CLANCHANNEL == var1.serverPacket) { // L: 6161
					var61 = var3.readByte(); // L: 6162
					var19 = var3.readString(); // L: 6163
				...
				if (ServerPacket.TRIGGER_ONDIALOGABORT == var1.serverPacket) { // L: 6197
				...
				if (ServerPacket.OBJ_COUNT == var1.serverPacket) { // L: 6202
					class21.processZoneUpdate(ZoneUpdate.OBJ_COUNT); // L: 6203
					var1.serverPacket = null; // L: 6204
					return true; // L: 6205
				}

				if (ServerPacket.UPDATE_INV_STOP_TRANSMIT == var1.serverPacket) { // L: 6207
					var17 = var3.readUShortLE(); // L: 6208
				...
				String var33;
				String var47;
				if (ServerPacket.MESSAGE_PRIVATE_ == var1.serverPacket) { // L: 6214
					var47 = var3.readString(); // L: 6215
				...
				if (ServerPacket.CLANCHANNEL_DELTA == var1.serverPacket) { // L: 6241
				...
				if (ServerPacket.UNKNOWN_NPC_UPDATE == var1.serverPacket) { // L: 6252
				...
				if (ServerPacket.NPC_INFO_SMALL == var1.serverPacket) { // L: 6258
				...
				Widget var52;
				if (ServerPacket.IF_SETNPCHEAD == var1.serverPacket) { // L: 6263
				...
				if (ServerPacket.UPDATE_INV_PARTIAL == var1.serverPacket) { // L: 6275
				...
				if (ServerPacket.IF_SETCOLOUR == var1.serverPacket) { // L: 6304
					var17 = var3.readIntLE(); // L: 6305
					var5 = var3.readShortAddLE(); // L: 6306
				...
				if (ServerPacket.UNKNOWN_NPC_UPDATE2 == var1.serverPacket) { // L: 6319
				...
				if (ServerPacket.URL_OPEN == var1.serverPacket) { // L: 6325
					...
					var53 = var66.readString(); // L: 6329
					class91.openURL(var53, true, false); // L: 6330
					var1.serverPacket = null; // L: 6331
					return true; // L: 6332
				}

				if (ServerPacket.CAM_MOVETO == var1.serverPacket) { // L: 6334
				...
				if (ServerPacket.IF_SETSCROLLPOS == var1.serverPacket) { // L: 6349
					var17 = var3.N_readIntME(); // L: 6350
					var5 = var3.readUShortLE(); // L: 6351
				...
				if (ServerPacket.LOGOUT == var1.serverPacket) { // L: 6364
				...
				if (ServerPacket.UPDATE_RUNENERGY == var1.serverPacket) { // L: 6370
					ItemComposition.method3406(); // L: 6371
					runEnergy = var3.readUnsignedByte(); // L: 6372
					System.out.println("UPDATE_RUNENERGY: " + runEnergy);
					field494 = cycleCntr; // L: 6373
					var1.serverPacket = null; // L: 6374
					return true; // L: 6375
				}

				if (ServerPacket.CHAT_FILTER_SETTINGS_PRIVATECHAT == var1.serverPacket) { // L: 6377
				...
				if (ServerPacket.IF_SETTEXT == var1.serverPacket) { // L: 6382
					var47 = var3.readString(); // L: 6383
					var5 = var3.N_readIntIME(); // L: 6384
				...
				if (ServerPacket.REBUILD_REGION == var1.serverPacket) { // L: 6393
					Canvas.loadRegions(true, var1.packetBuffer); // L: 6394
					var1.serverPacket = null; // L: 6395
					return true; // L: 6396
				}

				if (ServerPacket.UPDATE_REBOOT_TIMER == var1.serverPacket) { // L: 6398
					rebootTimer = var3.readUShortAdd() * 30; // L: 6399
					field494 = cycleCntr; // L: 6400
					var1.serverPacket = null; // L: 6401
					return true; // L: 6402
				}

				if (ServerPacket.UPDATE_UID192 == var1.serverPacket) { // L: 6404
				...
				if (ServerPacket.REFLECTION_CHECK == var1.serverPacket) { // L: 6410
				...
				if (ServerPacket.IF_SETHIDE == var1.serverPacket) {
					var17 = var3.readIntLE(); // HASH
					var56 = var3.readUnsignedByte() == 1; // HIDDE
					var52 = UserComparator9.getWidget(var17);
				...
				if (ServerPacket.IF_SETMODEL == var1.serverPacket) { // L: 6440
					var17 = var3.readInt(); // L: 6441
					var5 = var3.readUShortLE(); // L: 6442
				...
				if (ServerPacket.UPDATE_ZONE_FULL_FOLLOWS == var1.serverPacket) { // L: 6452
					Actor.ZONE_X = var3.readUByteAdd(); // L: 6453
					ItemComposition.Zone_Y = var3.readUnsignedByte(); // L: 6454

					for (var17 = ItemComposition.Zone_Y; var17 < ItemComposition.Zone_Y + 8; ++var17) { // L: 6455
						for (var5 = Actor.ZONE_X; var5 < Actor.ZONE_X + 8; ++var5) { // L: 6456
					...
					for (PendingSpawn var49 = (PendingSpawn)pendingSpawns.last(); var49 != null; var49 = (PendingSpawn)pendingSpawns.previous()) { // L: 6463 6464 6466
						if (var49.x >= ItemComposition.Zone_Y && var49.x < ItemComposition.Zone_Y + 8 && var49.y >= Actor.ZONE_X && var49.y < Actor.ZONE_X + 8 && var49.plane == GrandExchangeOfferTotalQuantityComparator.Client_plane) { // L: 6465
							var49.hitpoints = 0;
						}
					}
				...
				if (ServerPacket.LOGOUT_FULL == var1.serverPacket) { // L: 6471
					SequenceDefinition.logOut(); // L: 6472
					var1.serverPacket = null; // L: 6473
					return false; // L: 6474
				}

				if (ServerPacket.FRIENDLIST_LOADED == var1.serverPacket) { // L: 6476
				...
				if (ServerPacket.SYNTH_SOUND == var1.serverPacket) {
					var17 = var3.readUnsignedShort(); // sound id
					var5 = var3.readUnsignedByte(); // volume
					var18 = var3.readUnsignedShort(); // delay
					class128.queueSoundEffect(var17, var5, var18);
					var1.serverPacket = null;
					return true;
				}

				if (ServerPacket.CAM_RESET == var1.serverPacket) { // L: 6490
				...
				if (ServerPacket.RESET_CLIENT_VARCACHE == var1.serverPacket) { // L: 6496
				...
				if (ServerPacket.IF_OPENSUB == var1.serverPacket) { // L: 6509
					var17 = var3.readShortAddLE(); // COMPONENT
					var5 = var3.N_readIntME(); // CONTAINER
					var18 = var3.readUByteAdd(); // TYPE
					System.out.println("IF_OPEN_SUB");
					System.out.println("var17: "+var17);
					System.out.println("var5: "+var5);
				...
				if (ServerPacket.LOC_ADD_CHANGE == var1.serverPacket) { // L: 6519
					class21.processZoneUpdate(ZoneUpdate.LOC_ADD_CHANGE); // L: 6520
				...
				if (ServerPacket.UPDATE_INV_FULL == var1.serverPacket) {
					...
					var5 = var3.readUnsignedShort(); // container key
					if (var17 < -70000) {
						var5 += 32768;
					}
					...
					var7 = var3.readUnsignedShort(); // Slot count

					for (var8 = 0; var8 < var7; ++var8) {
						var9 = var3.readUByteNeg(); // amount
						if (var9 == 255) {
							var9 = var3.N_readIntME(); // overflow
						}

						var10 = var3.readUShortLE(); // ID
				...
				if (ServerPacket.REBUILD_REGION_NORMAL == var1.serverPacket) { // L: 6571
					Canvas.loadRegions(false, var1.packetBuffer); // L: 6572
					var1.serverPacket = null; // L: 6573
					return true; // L: 6574
				}

				if (ServerPacket.UPDATE_ZONE_PARTIAL_ENCLOSED == var1.serverPacket) { // L: 6576
					ItemComposition.Zone_Y = var3.readUByteAdd(); // L: 6577
					Actor.ZONE_X = var3.readUnsignedByte(); // L: 6578

					while (var3.offset < var1.serverPacketLength) { // L: 6579
						var17 = var3.readUnsignedByte(); // L: 6580
						ZoneUpdate var63 = class130.method2748()[var17]; // L: 6581
						class21.processZoneUpdate(var63); // L: 6582
					}
				...
				if (ServerPacket.UPDATE_SITESETTINGS == var1.serverPacket) { // L: 6587
					class128.method2728(var3.readString()); // L: 6588
					var1.serverPacket = null; // L: 6589
					return true; // L: 6590
				}

				if (ServerPacket.MAPPROJ_ANIM == var1.serverPacket) { // L: 6592
					class21.processZoneUpdate(ZoneUpdate.MAPPROJ_ANIM); // L: 6593
					var1.serverPacket = null; // L: 6594
					return true; // L: 6595
				}

				Widget var73;
				if (ServerPacket.IF1_MODELROTATE == var1.serverPacket) { // L: 6597
					var17 = var3.readUShortAdd(); // L: 6598
					var5 = var3.readUShortLE(); // L: 6599
					var18 = var3.N_readIntIME(); // L: 6600
				...
				if (ServerPacket.SEND_PING == var1.serverPacket) { // L: 6606
					var17 = var3.readInt(); // L: 6607
					var5 = var3.readInt(); // L: 6608
					var18 = Occluder.getGcDuration(); // L: 6609
					PacketBufferNode var75 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.SEND_PING_REPLY, packetWriter.isaacCipher); // L: 6611
					var75.packetBuffer.writeByte(GameEngine.fps); // L: 6612
					var75.packetBuffer.N_writeIntME(var17); // L: 6613
					var75.packetBuffer.N_writeIntME(var5); // L: 6614
					var75.packetBuffer.writeByteSub(var18); // L: 6615
				...
				boolean var68;
				if (ServerPacket.TRADING_POST_RESULTS == var1.serverPacket) { // L: 6620
				...
				if (ServerPacket.IF_SETANIM == var1.serverPacket) { // L: 6631
					var17 = var3.readShortAdd(); // L: 6632
					var5 = var3.readIntLE(); // L: 6633
				...
				long var37;
				if (ServerPacket.MESSAGE_FRIENDCHANNEL == var1.serverPacket) { // L: 6644
				...
				if (ServerPacket.RESET_ANIMS == var1.serverPacket) { // L: 6670
				...
				Widget var36;
				if (ServerPacket.IF_MOVESUB == var1.serverPacket) { // L: 6680
					var17 = var3.readIntLE(); // FROM
					...
					if (var36 != null) { // L: 6693
						...
						MouseRecorder.revalidateWidgetScroll(WorldMapSection1.Widget_interfaceComponents[var36.hash >>> 16], var36, true); // L: 6695
					}
				...
				if (ServerPacket.CHAT_FILTER_SETTINGS == var1.serverPacket) { // L: 6701
					publicChatMode = var3.readUByteNeg(); // L: 6702
					tradeChatMode = var3.readUByteSub(); // L: 6703
					var1.serverPacket = null; // L: 6704
					return true; // L: 6705
				}

				if (ServerPacket.SET_MAP_FLAG == var1.serverPacket) {
					destinationX = var3.readUnsignedByte(); // x
					if (destinationX == 255) {
						destinationX = 0;
					}

					destinationY = var3.readUnsignedByte(); // y
					if (destinationY == 255) {
						destinationY = 0;
					}
				...
				if (ServerPacket.UPDATE_STAT == var1.serverPacket) {
					...
					var17 = var3.readUByteSub(); // SKILL
					var5 = var3.N_readIntME(); // XP
					var18 = var3.readUByteAdd(); // LEVEL
				...
				if (ServerPacket.CLANSETTINGS_DELTA == var1.serverPacket) { // L: 6728
				...
				if (ServerPacket.IF_SETEVENTS == var1.serverPacket) { // L: 6739
					var17 = var3.N_readIntIME(); // SETTING
					var5 = var3.readInt(); // HASH
					var18 = var3.readUnsignedShort(); // TO
					System.out.println("IF_SET_EVENTS: " + "SETTING: " + var17 + " HASH: " + var5  + " TO:" + var18);

					if (var18 == 65535) { // L: 6743
						var18 = -1;
					}

					var7 = var3.readUShortAdd(); // FROM
				...
				if (ServerPacket.CLANSETTINGS_FULL == var1.serverPacket) { // L: 6755
				...
				if (ServerPacket.IF_OPENTOP == var1.serverPacket) { // L: 6773
					var17 = var3.readShortAddLE(); // L: 6774
					System.out.println("IF_OPEN_TOP: " + var17);
				...
				Widget var59;
				if (ServerPacket.IF_SETPLAYERHEAD == var1.serverPacket) { // L: 6783
					var17 = var3.N_readIntIME(); // L: 6784
				...
				if (ServerPacket.IF_CLOSESUB == var1.serverPacket) { // L: 6792
				...
				if (ServerPacket.PREFETCH_GAMEOBJECTS == var1.serverPacket) { // L: 6803
					class21.processZoneUpdate(ZoneUpdate.PREFETCH_GAMEOBJECTS); // L: 6804
					var1.serverPacket = null; // L: 6805
					return true; // L: 6806
				}

				if (ServerPacket.UPDATE_FRIENDLIST == var1.serverPacket) { // L: 6808
				...
				if (ServerPacket.MINIMAP_TOGGLE == var1.serverPacket) { // L: 6814
					minimapState = var3.readUnsignedByte(); // L: 6815
					var1.serverPacket = null; // L: 6816
					return true; // L: 6817
				}

				if (ServerPacket.MESSAGE_PRIVATE_ECHO == var1.serverPacket) { // L: 6819
					var47 = var3.readString(); // L: 6820
				...
				if (ServerPacket.LOC_ANIM == var1.serverPacket) { // L: 6826
					class21.processZoneUpdate(ZoneUpdate.LOC_ANIM); // L: 6827
					var1.serverPacket = null; // L: 6828
					return true; // L: 6829
				}

				if (ServerPacket.LOGOUT_TRANSFER == var1.serverPacket) { // L: 6831
					World var48 = new World(); // L: 6832
					var48.host = var3.readString(); // L: 6833
				...
				if (ServerPacket.HEAT_MAP == var1.serverPacket) { // L: 6844
				...
				if (ServerPacket.OBJ_ADD == var1.serverPacket) { // L: 6853
					class21.processZoneUpdate(ZoneUpdate.OBJ_ADD); // L: 6854
					var1.serverPacket = null; // L: 6855
					return true; // L: 6856
				}

				if (ServerPacket.LOC_DEL == var1.serverPacket) { // L: 6858
					class21.processZoneUpdate(ZoneUpdate.LOC_DEL); // L: 6859
					var1.serverPacket = null; // L: 6860
					return true; // L: 6861
				}

				if (ServerPacket.IF_SETOBJECT == var1.serverPacket) { // L: 6863
					var17 = var3.readShortAddLE(); // ITEM ID
					if (var17 == 65535) { // L: 6865
						var17 = -1;
					}

					var5 = var3.readInt(); // HASH
					var18 = var3.N_readIntME(); // AMOUNT
				...
				if (ServerPacket.VARP_LARGE == var1.serverPacket) { // L: 6902
					var17 = var3.readIntLE();
					var5 = var3.readUnsignedShort();
					System.out.println("Varp_Large");
					System.out.println("var17:"+var17);
					System.out.println("var5:"+var5);
				...
				if (ServerPacket.MAP_ANIM == var1.serverPacket) { // L: 6914
					class21.processZoneUpdate(ZoneUpdate.MAP_ANIM); // L: 6915
					var1.serverPacket = null; // L: 6916
					return true; // L: 6917
				}

				if (ServerPacket.TOGGLE_OCULUS_ORB == var1.serverPacket) { // L: 6919
				...
				if (ServerPacket.CLANCHANNEL_FULL == var1.serverPacket) { // L: 6928
				...
				long var42;
				if (ServerPacket.MESSAGE_PRIVATE == var1.serverPacket) { // L: 6946
					var47 = var3.readString(); // L: 6947
				...
				if (ServerPacket.VARCLAN == var1.serverPacket) { // L: 6973
					if (NPC.field1210 == null) { // L: 6974
						NPC.field1210 = new class353(ZoneUpdate.HitSplatDefinition_cachedSprites);
					}

					class408 var46 = ZoneUpdate.HitSplatDefinition_cachedSprites.method6391(var3); // L: 6975
				...
				if (ServerPacket.IF_SETPOSITION == var1.serverPacket) { // L: 6981
					var17 = var3.N_readIntME(); // L: 6982
					var5 = var3.readShortLE(); // L: 6983
					var18 = var3.readShortLE(); // L: 6984
					var73 = UserComparator9.getWidget(var17); // L: 6985
					System.out.println("IF_SET_POSTION: + var17: " + var17 + " var5: " + var5 + " var18:" + var18);
				...
				if (ServerPacket.GAMEFRAME_FULL == var1.serverPacket) { // L: 6998
				...
				if (ServerPacket.NPC_INFO_LARGE == var1.serverPacket) { // L: 7041
					class16.updateNpcs(true, var3); // L: 7042
					var1.serverPacket = null; // L: 7043
					return true; // L: 7044
				}

				if (ServerPacket.MIDI_SONG == var1.serverPacket) { // L: 7046
					var17 = var3.readUShortAdd(); // L: 7047
				...
				if (ServerPacket.MIDI_JINGLE == var1.serverPacket) { // L: 7053
				...
				if (ServerPacket.UPDATE_FRIENDCHAT_CHANNEL_FULL == var1.serverPacket) { // L: 7061
				...
				if (ServerPacket.ENTER_FREECAM == var1.serverPacket) { // L: 7075
				...
				if (ServerPacket.SYNC_CLIENT_VARCACHE == var1.serverPacket) { // L: 7081
				...
				if (ServerPacket.VARCLAN_DISABLE == var1.serverPacket) { // L: 7099
					NPC.field1210 = null; // L: 7100
					var1.serverPacket = null; // L: 7101
					return true; // L: 7102
				}

				if (ServerPacket.UPDATE_INV_CLEAR == var1.serverPacket) { // L: 7104
					var17 = var3.N_readIntME(); // L: 7105
				...
				if (ServerPacket.UPDATE_ZONE_PARTIAL_FOLLOWS == var1.serverPacket) { // L: 7115
					ItemComposition.Zone_Y = var3.readUnsignedByte(); // L: 7116
					Actor.ZONE_X = var3.readUByteNeg(); // L: 7117
					var1.serverPacket = null; // L: 7118
					return true; // L: 7119
				}

				if (ServerPacket.PLAYER_INFO == var1.serverPacket) { // L: 7121
				...
				if (ServerPacket.VARCLAN_ENABLE == var1.serverPacket) { // L: 7127
					NPC.field1210 = new class353(ZoneUpdate.HitSplatDefinition_cachedSprites); // L: 7128
					var1.serverPacket = null; // L: 7129
					return true; // L: 7130
				}

				if (ServerPacket.IF_SETANGLE == var1.serverPacket) { // L: 7132
					var17 = var3.readUnsignedShort(); // L: 7133
					var5 = var3.readUnsignedShort(); // L: 7134
					var18 = var3.readUnsignedShort(); // L: 7135
					var7 = var3.N_readIntME(); // L: 7136
					var36 = UserComparator9.getWidget(var7); // L: 7137
					System.out.println("IF_SET_ANGLE");
					System.out.println("var17" + var17);
					System.out.println("var5" + var5);
					System.out.println("var18" + var18);
				...
				if (ServerPacket.HINT_ARROW == var1.serverPacket) { // L: 7147
				...
				if (ServerPacket.UPDATE_FRIENDCHAT_CHANNEL_SINGLEUSER == var1.serverPacket) { // L: 7180
	...
	final void method1186() {
		class184.invalidateWidget(source); // L: 11190
		...
		if (field680 && field648) { // L: 11192
			...
			if (var1 + source.width > field649 + clickedWidgetParent.width) { // L: 11203
				var1 = field649 + clickedWidgetParent.width - source.width;
			}
			...
			if (var2 + source.height > field650 + clickedWidgetParent.height) { // L: 11205
				var2 = field650 + clickedWidgetParent.height - source.height;
			}

			int var3 = var1 - field652; // L: 11206
			int var4 = var2 - field468; // L: 11207
			int var5 = source.dragZoneSize; // L: 11208
			if (class134.widgetDragDuration > source.dragThreshold && (var3 > var5 || var3 < -var5 || var4 > var5 || var4 < -var5)) { // L: 11209 11210
			...
			ScriptEvent var8;
			if (source.onDrag != null && isDraggingWidget) { // L: 11216
				var8 = new ScriptEvent(); // L: 11217
				var8.widget = source; // L: 11218
				var8.mouseX = var6; // L: 11219
				var8.mouseY = var7; // L: 11220
				var8.args = source.onDrag; // L: 11221
				Tile.runScriptEvent(var8); // L: 11222
			}

			if (MouseHandler.MouseHandler_currentButton == 0) { // L: 11224
				if (isDraggingWidget) { // L: 11225
					if (source.onDragComplete != null) { // L: 11226
						var8 = new ScriptEvent(); // L: 11227
						var8.widget = source; // L: 11228
						var8.mouseX = var6; // L: 11229
						var8.mouseY = var7; // L: 11230
						var8.dragTarget = destination; // L: 11231
						var8.args = source.onDragComplete; // L: 11232
						Tile.runScriptEvent(var8); // L: 11233
					}

					if (destination != null && BuddyRankComparator.method2501(source) != null) { // L: 11235
						PacketBufferNode var12 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.IF_BUTTOND, packetWriter.isaacCipher); // L: 11237
						var12.packetBuffer.writeShortLE(destination.item); // L: 11238
						var12.packetBuffer.writeShort(destination.slot); // L: 11239
						var12.packetBuffer.writeShortLE(source.slot); // L: 11240
						var12.packetBuffer.N_writeIntME(destination.hash); // L: 11241
						var12.packetBuffer.writeShortLE(source.item); // L: 11242
						var12.packetBuffer.N_writeIntIME(source.hash); // L: 11243
