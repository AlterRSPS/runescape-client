...
public class class114 extends class116 {
	...
	static int method2542(int var0, Script var1, boolean var2) {
		if (var0 == ScriptOpcodes.CHAT_GETFILTER_PUBLIC) { // L: 3317
			...
			PacketBufferNode var13 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.SET_CHATFILTERSETTINGS, Client.packetWriter.isaacCipher); // L: 3328
			...
			if (var0 == ScriptOpcodes.CHAT_SENDABUSEREPORT) { // L: 3335
				...
				var6 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.SEND_SNAPSHOT, Client.packetWriter.isaacCipher); // L: 3341
				var6.packetBuffer.writeByte(Client.stringCp1252NullTerminatedByteSize(var8) + 2); // L: 3342
				var6.packetBuffer.sendStringByBytes(var8); // L: 3343
				...
				if (var0 == ScriptOpcodes.CHAT_GETHISTORY_BYTYPEANDLINE) { // L: 3349
					...
					if (var0 == ScriptOpcodes.CHAT_GETHISTORY_BYUID) { // L: 3372
						...
						if (var0 == ScriptOpcodes.CHAT_SENDPUBLIC) { // L: 3398
							...
							var12 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.MESSAGE_PRIVATE, Client.packetWriter.isaacCipher); // L: 3410
							var12.packetBuffer.writeShort(0); // L: 3411
							int var10 = var12.packetBuffer.offset; // L: 3412
							var12.packetBuffer.sendStringByBytes(var8); // L: 3413
	...
	static final void Widget_addToMenu(Widget var0, int var1, int var2) {
		if (var0.buttonType == 1) { // L: 10471
			EnumComposition.insertMenuItemNoShift(var0.buttonText, "", 24, 0, 0, var0.hash); // L: 10472
		}
		...
		if (var0.buttonType == 2 && !Client.isSpellSelected) { // L: 10475
			...
			if (var3 != null) { // L: 10477
				EnumComposition.insertMenuItemNoShift(var3, MouseHandler.colorStartTag(65280) + var0.spellName, 25, 0, -1, var0.hash); // L: 10478
			}
		}

		if (var0.buttonType == 3) { // L: 10482
			EnumComposition.insertMenuItemNoShift("Close", "", 26, 0, 0, var0.hash); // L: 10483
		}

		if (var0.buttonType == 4) { // L: 10486
			EnumComposition.insertMenuItemNoShift(var0.buttonText, "", 28, 0, 0, var0.hash); // L: 10487
		}

		if (var0.buttonType == 5) { // L: 10490
			EnumComposition.insertMenuItemNoShift(var0.buttonText, "", 29, 0, 0, var0.hash); // L: 10491
		}

		if (var0.buttonType == 6 && Client.meslayerContinueWidget == null) { // L: 10494
			EnumComposition.insertMenuItemNoShift(var0.buttonText, "", 30, 0, -1, var0.hash); // L: 10495
		}
					...
					if (var1 >= var6 && var2 >= var7 && var1 < var6 + 32 && var2 < var7 + 32) { // L: 10508
						...
						if (var0.itemIds[var13] > 0) { // L: 10511
							...
							if (Client.isItemSelected == 1 && Tiles.method2045(PendingSpawn.getWidgetFlags(var0))) { // L: 10513
								if (var0.hash != MouseHandler.component || var13 != ReflectionCheck.clickedSlot) { // L: 10514
									EnumComposition.insertMenuItemNoShift("Use", Client.selectedItemName + " " + "->" + " " + MouseHandler.colorStartTag(16748608) + var8.name, 31, var8.id, var13, var0.hash); // L: 10515
								}
								...
								if ((class116.selectedSpellFlags & 16) == 16) { // L: 10520
									EnumComposition.insertMenuItemNoShift(Client.selectedSpellActionName, Client.selectedSpellName + " " + "->" + " " + MouseHandler.colorStartTag(16748608) + var8.name, 32, var8.id, var13, var0.hash); // L: 10521
								}
							} else {
								...
								if (class111.method2514(PendingSpawn.getWidgetFlags(var0))) { // L: 10537
									EnumComposition.insertMenuItemNoShift("Use", MouseHandler.colorStartTag(16748608) + var8.name, 38, var8.id, var13, var0.hash); // L: 10538
								}
								...
								if (var9 != null) { // L: 10551
									for (var11 = 4; var11 >= 0; --var11) { // L: 10552
										if (var9[var11] != null) { // L: 10553
											...
											EnumComposition.insertMenuItemNoShift(var9[var11], MouseHandler.colorStartTag(16748608) + var8.name, var12, var8.id, var13, var0.hash); // L: 10560
										}
									}
								}

								EnumComposition.insertMenuItemNoShift("Examine", MouseHandler.colorStartTag(16748608) + var8.name, 1005, var8.id, var13, var0.hash); // L: 10565
							}
						}
					}
		...
		if (var0.isIf3) { // L: 10574
			if (Client.isSpellSelected) { // L: 10575
				if (class19.method279(PendingSpawn.getWidgetFlags(var0)) && (class116.selectedSpellFlags & 32) == 32) { // L: 10576
					EnumComposition.insertMenuItemNoShift(Client.selectedSpellActionName, Client.selectedSpellName + " " + "->" + " " + var0.dataText, 58, 0, var0.slot, var0.hash); // L: 10577
				}
			} else {
				for (var13 = 9; var13 >= 5; --var13) { // L: 10582
					String var14 = SoundSystem.method785(var0, var13); // L: 10583
					if (var14 != null) { // L: 10584
						EnumComposition.insertMenuItemNoShift(var14, var0.dataText, 1007, var13 + 1, var0.slot, var0.hash); // L: 10585
					}
				}

				var3 = class20.Widget_getSpellActionName(var0); // L: 10589
				if (var3 != null) { // L: 10590
					EnumComposition.insertMenuItemNoShift(var3, var0.dataText, 25, 0, var0.slot, var0.hash); // L: 10591
				}

				for (var4 = 4; var4 >= 0; --var4) { // L: 10594
					String var5 = SoundSystem.method785(var0, var4); // L: 10595
					if (var5 != null) { // L: 10596
						ModelData0.insertMenuItem(var5, var0.dataText, 57, var4 + 1, var0.slot, var0.hash, var0.prioritizeMenuEntry); // L: 10597
					}
				}

				if (ClientPacket.method4710(PendingSpawn.getWidgetFlags(var0))) { // L: 10601
					EnumComposition.insertMenuItemNoShift("Continue", "", 30, 0, var0.slot, var0.hash); // L: 10602
				}
			}
		}
